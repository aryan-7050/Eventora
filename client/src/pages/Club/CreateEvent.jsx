...
const CreateEvent = () => {
    ...
    useEffect(() => {
        ...
        if (storedUser) {
            ...
            setFormData(prev => ({ ...prev, contactEmail: storedUser.email }));
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.isPaid && !formData.registrationAmount) {
            return toast.warn('Registration amount is required for paid events');
        }

        if (!clubInfo.id) {
            setLoading(false);
            return toast.error('Club ID is missing. Please log out and log in again.');
        }
    ...
    return (
        <div className="create-event-container animate-fade-in">
            <div className="dashboard-header">
                ...
                <p className="dashboard-subtitle">Fill in the details to propose a new event to the administration.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="event-form glass-panel">
                ...
                <div className="form-actions">
                    <button type="button" className="btn btn-secondary" onClick={() => navigate('/club/dashboard')}>Cancel</button>
                    <button type="submit" className="btn btn-club create-btn" disabled={loading}>
                        {loading ? 'Processing...' : 'Propose Event'}
                    </button>
                </div>
            </form>
        </div>
